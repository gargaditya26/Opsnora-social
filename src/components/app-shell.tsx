...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const auth=useAuth(); const {user}=auth;
  ...
  return <div className={`app ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <div className="main-area">
      <header className="topbar">
        ...
        <div className="top-actions"><div className="notification-wrap"><button className="icon-button notification-button" aria-label="Notifications" onClick={()=>{setNotificationsOpen(v=>!v);setProfileOpen(false)}}><Bell size={19}/>{unread>0&&<i/>}</button>{notificationsOpen&&<div className="notification-panel"><header><strong>Notifications</strong><button onClick={markNotificationsRead}>Mark all read</button></header>{notifications.length?notifications.slice(0,5).map(n=><article key={n.id} className={n.read?"":"unread"}><strong>{n.title}</strong><p>{n.body}</p></article>):<p className="no-notifications">You&apos;re all caught up.</p>}</div>}</div><Link href="/create" className="button button-primary quick-create"><Plus size={17}/> <span>Quick create</span></Link><div className="profile-wrap"><button className="avatar" onClick={() => {setProfileOpen(v => !v);setNotificationsOpen(false)}}>{(user?.displayName||user?.email||"AG").split(/\s|@/).map(x=>x[0]).join("").slice(0,2).toUpperCase()}</button>{profileOpen && <div className="profile-menu"><div><strong>{user?.displayName||"Aditya Garg"}</strong><small>{user?.email||"Local demo session"}</small></div><Link href="/settings?tab=profile">Profile</Link><Link href="/settings?tab=workspace">Workspace settings</Link><Link href="/settings?tab=security">Account settings</Link><button onClick={async()=>{await auth.signOut();router.replace("/login")}}><LogOut size={15}/> Sign out</button></div>}</div></div>
      </header>
      <main>{children}</main>
    </div>
  </div>;
