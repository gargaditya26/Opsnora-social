...
import { defaultBrandProfiles, demoPosts, workspaces } from "@/lib/demo-data";
import type { BrandProfile, Platform, Post, PostStatus } from "@/lib/types";
...
interface StoreValue {
  ready: boolean; posts: Post[]; workspaceId: string; brandProfile: BrandProfile; connections: ConnectionMap; notifications: AppNotification[]; media: MediaItem[]; members: TeamMember[];
  ...
  updateBrandProfile(patch: Partial<Omit<BrandProfile,"workspaceId"|"updatedAt">>): void;
}
...
export function AppStoreProvider({children}:{children:React.ReactNode}){
 ...
 const [brandProfiles,setBrandProfiles]=useState<Record<string,BrandProfile>>(defaultBrandProfiles);
 useEffect(()=>{try{const raw=localStorage.getItem(STORAGE_KEY);if(raw){const s=JSON.parse(raw);setPosts(s.posts??demoPosts);setWorkspaceId(s.workspaceId??workspaces[0].id);setBrandProfiles({...defaultBrandProfiles,...(s.brandProfiles??{})});setConnections(s.connections??{instagram:"expired",linkedin:"connected"});setNotifications(s.notifications??[]);setMedia(s.media??initialMedia);setMembers(s.members??[]);}}finally{setReady(true)}},[]);
 useEffect(()=>{if(ready)localStorage.setItem(STORAGE_KEY,JSON.stringify({posts,workspaceId,brandProfiles,connections,notifications,media,members}))},[ready,posts,workspaceId,brandProfiles,connections,notifications,media,members]);
 const brandProfile=useMemo(()=>brandProfiles[workspaceId]??defaultBrandProfiles[workspaceId]??{...defaultBrandProfiles.ws_opsnora,workspaceId,brandName:"New brand"},[brandProfiles,workspaceId]);
 const value=useMemo<StoreValue>(()=>({ready,posts,workspaceId,brandProfile,connections,notifications,media,members,setWorkspaceId,
  ...
  updateBrandProfile:(patch)=>setBrandProfiles(v=>({...v,[workspaceId]:{...brandProfile,...patch,workspaceId,updatedAt:new Date().toISOString()}}))
 }),[ready,posts,workspaceId,brandProfile,connections,notifications,media,members]);
