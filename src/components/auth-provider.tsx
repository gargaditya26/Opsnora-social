...
export function AuthProvider({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{void initializeFirebaseAnalytics();if(!firebaseAuth){if(firebaseConfigurationError)console.error(firebaseConfigurationError);setDemoAccess(process.env.NEXT_PUBLIC_DEMO_MODE==="true"&&sessionStorage.getItem("opsnora-demo-access")==="true");setLoading(false);return}void setPersistence(firebaseAuth,browserLocalPersistence).catch(()=>undefined);return onAuthStateChanged(firebaseAuth,value=>{setUser(value);setLoading(false)})},[]);
 ...
 return <AuthContext.Provider value={{user,loading,isAuthenticated:Boolean(user),configured:isFirebaseConfigured,configurationError:isFirebaseConfigured?null:"Authentication is temporarily unavailable while the service is being configured. Please try again later.",demoAccess,signIn,signUp,signOut,resetPassword,enableDemoAccess}}>{children}</AuthContext.Provider>
}
