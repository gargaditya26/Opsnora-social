...
import { firebaseConfig, isFirebaseConfigured, isFirebaseStorageConfigured } from "./firebase-env";
export { firebaseConfigurationError, isFirebaseConfigured, isFirebaseStorageConfigured, missingFirebaseEnv } from "./firebase-env";
const app = isFirebaseConfigured ? (getApps().length ? getApp() : initializeApp(firebaseConfig)) : null;
export const firebaseAuth = app ? getAuth(app) : null;
export const firebaseDb = app ? getFirestore(app) : null;
export const firebaseStorage = app && isFirebaseStorageConfigured ? getStorage(app) : null;
...
export async function uploadWorkspaceMedia(file: File, workspaceId: string, uid: string, onProgress?: (percent: number) => void) {
  if (!firebaseStorage) throw new Error("Cloud media storage is not enabled.");
